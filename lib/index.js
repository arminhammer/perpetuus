...
var removeAttrFromSelector = function(selector) {
  console.log('Sel: ', selector);
  console.log('Sel new: ', selector.replace(/@\w+$/i, ''));
  return selector.replace(/@\w+$/i, '');
};
  ...
  var promise = new BPromise(function(resolve, reject) {
    ...
    if(template.submit) {
      ...
      Object.keys(template.submit.input).forEach(function(key) {
        ...
        //console.log('scrape');
        var qName = 'input[name='+key+']';
        console.log(qName);
        scrape = scrape.setValue(qName, template.submit.input[key]);
      });

      console.log('Submitting');

      scrape = scrape
        .waitForExist(template.submit.button, 30000)
        .click(template.submit.button);

      if(template.submit.waitFor) {
        console.log('waitFor found');
        scrape = scrape.waitForExist(template.submit.waitFor, 30000);
      } else {
        var waitCond = removeAttrFromSelector(template.submit.val[Object.keys(template.submit.val)[0]][0]);
        console.log('waiting for ', waitCond);
        scrape = scrape.waitForExist(waitCond, 30000);
      }
